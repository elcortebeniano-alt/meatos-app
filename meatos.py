    ...
    with col_der:
        ...
        if st.session_state['carrito']:
            ...
            if b2.button("✅ COBRAR", type="primary", disabled=not cobrar):
                ...
                link = f"https://wa.me/591{cel.strip()}?text={msg}" if cel else f"https://wa.me/?text={msg}"
                st.session_state['ultimo_ticket'] = {'link': link, 'texto': "📲 ENVIAR TICKET"}
                st.session_state['carrito'] = []
                st.balloons(); st.success("¡Cobrado!"); time.sleep(3); st.rerun()
