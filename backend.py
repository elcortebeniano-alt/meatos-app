...
# --- TICKET ACTUALIZADO CON CLIENTE ---
def generar_html_ticket(carrito, total, fecha, metodo, recibo_id, direccion, telefono, vendedor, cliente_nombre=""):
    ...
    # Si hay cliente, mostramos su nombre
    cliente_html = f'<p style="margin:1px 0;">Cliente: {cliente_nombre}</p>' if cliente_nombre else ""
    ...
    <head>
        ...
        <style>
            ...
            .section-title {{ font-size: 10px; font-weight: bold; text-transform: uppercase; margin: 5px 0; border-bottom: 1px solid #000; text-align: left; }}
            .totals-box {{ margin-top: 10px; text-align: right; border-top: 1px solid #000; padding-top: 5px; }}
            .total-big {{ font-size: 14px; font-weight: bold; color: #000; }}
            .footer {{ text-align: center; margin-top: 15px; font-size: 9px; color: #444; }}
            .no-print {{ text-align: center; margin-bottom: 10px; padding-top: 5px; width: 100%; }}
            button {{ background:#000; color:#fff; border:none; padding:5px 10px; border-radius:4px; font-size:10px; cursor: pointer; }}
