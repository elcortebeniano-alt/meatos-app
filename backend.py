...
def limpiar_fechas(df):
    if 'Fecha' in df.columns: 
        df['Fecha'] = df['Fecha'].astype(str).replace("NaT", "").fillna("")
    return df
...
# --- TICKET 80mm (ANCHO Y LEGIBLE) ---
def generar_html_ticket(carrito, total, fecha, metodo, recibo_id, direccion, telefono, vendedor, cliente_nombre=""):
    
    logo_b64 = get_image_base64("Logo-Final.png")
    img_tag = f'<img src="{logo_b64}" alt="Logo" style="width: 80px; height: auto;">' if logo_b64 else ""

    cliente_html = f'<p style="margin:2px 0; font-weight:bold;">Cliente: {cliente_nombre}</p>' if cliente_nombre else ""

    items_html = ""
    for item in carrito:
        items_html += f"""
        <div style="margin-bottom: 8px; border-bottom: 1px dashed #ccc; padding-bottom: 5px;">
            <div style="font-weight: bold; font-size: 14px; color: #000; text-align: left;">{item['Producto']}</div>
            <div style="display: flex; justify-content: space-between; font-size: 12px; color: #333;">
                ...
                <div style="font-weight: bold; font-size: 13px;">{item['Subtotal']:.2f}</div>
            </div>
        </div>
        """
    ...
    <head>
        <meta charset="UTF-8">
        <style>
            @page {{ margin: 0; size: 80mm auto; }} /* CAMBIO A 80mm */
            ...
            .ticket-body {{ width: 72mm; padding: 5px; box-sizing: border-box; }} /* ANCHO UTIL */
            ...
            .logo-box {{ flex: 0 0 70px; }}
            .info-box {{ flex: 1; text-align: right; padding-left: 5px; }}
            .biz-name {{ font-size: 16px; font-weight: bold; margin: 0; text-transform: uppercase; color: #000; }}
            .biz-meta {{ font-size: 11px; margin: 2px 0; display: block; color: #444; }}
            .recibo-id {{ font-size: 12px; font-weight: bold; margin-top: 4px; display: block; color: #000; }}
            .section-title {{ font-size: 12px; font-weight: bold; text-transform: uppercase; margin: 8px 0; border-bottom: 1px solid #000; text-align: left; }}
            .totals-box {{ margin-top: 15px; text-align: right; border-top: 2px solid #000; padding-top: 8px; }}
            .total-big {{ font-size: 20px; font-weight: bold; color: #000; }}
            ...
            button {{ background:#000; color:#fff; border:none; padding:8px 15px; border-radius:6px; font-size:14px; cursor: pointer; }}
            @media print {{ .no-print {{ display: none; }} }}
        </style>
    </head>
    <body>
        <div class="no-print"><button onclick="window.print()">🖨️ IMPRIMIR TICKET (80mm)</button></div>
        <div class="ticket-body">
            ...
            <div class="totals-box">
                ...
                <div style="font-size: 12px;">Pago: {metodo}</div>
            </div>
            <div class="footer">
                <p style="margin:0; font-weight:bold;">¡Gracias por su preferencia!</p>
                <p style="margin:2px 0;">Le atendió: {vendedor}</p>
